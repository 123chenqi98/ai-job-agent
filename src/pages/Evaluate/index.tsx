...
import { saveEvalRecord } from '@/data/evaluationHistory'
...
export default function Evaluate() {
  ...
  const [result, setResult] =
    useState<(JdMatchResponse & { company?: string; title?: string; jd: string }) | null>(null)
  ...
  const runMatch = async () => {
    ...
    try {
      ...
      if (!controller.signal.aborted) {
        saveEvalRecord({ ...meta, jd: jd.trim(), match: res.match, source: 'single' })
        setResult({ ...res, ...meta, jd: jd.trim() })
      }
    } catch (err) {
